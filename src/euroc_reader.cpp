...
bool EurocReader::load() {
  ...
  get_logger()->info("Loaded: {} IMU, {} stereo, {} GT, {} total events", imu_data_.size(),
                     stereo_timestamps_.size(), ground_truth_.size(), events_.size());
  return true;
}
...
// ---------------------------------------------------------------------------
bool EurocReader::loadCameraTimestamps(const std::string& cam_dir,
...
bool EurocReader::loadGroundTruth() {
  ...
  while (std::getline(ifs, line)) {
    ...
    gt.q = Eigen::Quaterniond(std::stod(tokens[4]),   // w
                              std::stod(tokens[5]),   // x
                              std::stod(tokens[6]),   // y
                              std::stod(tokens[7]));  // z
...
// ---------------------------------------------------------------------------
void EurocReader::replay(const ImuCallback& on_imu, const StereoCallback& on_stereo) const {
